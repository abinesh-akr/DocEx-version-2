...
const API_URL = import.meta.env.VITE_API_URL || 'https://flask-7hba.onrender.com';
...
export const uploadGate = async (formData) => {
    try {
        ...
        const response = await axios.post(`${API_URL}/api/gate`, formData, {
            headers: {
                'Access-Control-Allow-Origin' : 'https://docex-version-2.onrender.com/api/gate',
...
export const uploadAadhar = async (formData) => {
    try {
        ...
        const response = await axios.post(`${API_URL}/api/aadhar`, formData, {
            headers: {
                'Access-Control-Allow-Origin' : 'https://flask-7hba.onrender.com/api/aadhar',
                'Content-Type': 'multipart/form-data'
            },
        });
        console.log(response.data)
